    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if(isinstance(expr1, Sum)):
        if(isinstance(expr2, Sum)):
            for a, b in itertools.product(expr1, expr2):
                result = Sum([result, Product([a,b])])
                # result = Sum([result, Product([a,b])])
        #SUM * PROD
        else:
            for factor in expr1:
                ...
                #result = Sum([result, Product(list(factor) + list(expr2))])
    else:
        if(isinstance(expr2, Sum)):
            for factor in expr2:
                for i in expr1:
                    factor *= i
                result.append(factor)
                #result = Sum([result, Product(list(factor) + list(expr1))])  
        else:
            #PROD * PROD
