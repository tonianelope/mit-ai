...
brother = IF( AND('sibling (?x) (?y)',
                  'male (?x)'),
              THEN ('brother (?x) (?y)'))

sister = IF( AND('sibling (?x) (?y)',
                  'female (?x)'),
              THEN ('sister (?x) (?y)'))

cousin = IF( AND('parent (?x) (?z)',
                 'parent (?y) (?w)',
                 'sibling (?x) (?y)'),
             THEN('cousin (?z) (?w)'))
...
# family_rules.
family_rules = [same, sibling,
                father, mother, son, daughter,
                sister, brother,
                cousin,
                grandparent, grandchild]                    # fill me in
...
HOW_MANY_HOURS_THIS_PSET_TOOK = '4' #to long for 2nd
WHAT_I_FOUND_INTERESTING = 'Wrapping my head around the backchaining, and the family rules (how to define sane)'
